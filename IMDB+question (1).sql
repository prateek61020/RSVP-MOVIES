...
-- checking shape and null values for each table. 
...
-- year wise movie production is decreased over period of 3 year, 
...
-- To fetch list of movies produced in USA or India
SELECT 
    country, COUNT(id) AS total_moives
FROM
    movie
WHERE (country like '%USA%' or country like '%India%') and year = 2019
GROUP BY country
order by total_moives desc;

-- To get sum of movies produced in USA or India 
select sum(a.number_of_movies) as 'Movies produced in the USA or India in the year 2019 ' 
from (select country,count(*) AS number_of_movies
		from movie
...
(
SELECT 
    movie_id, COUNT(movie_id) AS total_id
FROM
	genre
GROUP BY movie_id
HAVING total_id = 1
)
SELECT count(movie_id) AS total_movie_with_one_genre
FROM only_one_genre;
...
-- 104.32 mins average for all the movies. action movies 112.88 mins and darma movies 106.77.
...
-- fetching the list of genre with there movie counts
SELECT genre, 
...
-- fetching the record with genre = 'Thriller
...
SELECT 
	...
	-- DENSE_RANK() over(order by COUNT(id) desc) as movie_dense_rank
FROM 
	movie m 
		INNER JOIN 
...
FROM
    ...
    ratings r ON r.movie_id = m.id
WHERE
    title LIKE 'The%' AND avg_rating > 8.0
...
WHERE
    median_rating = 8
        AND date_published BETWEEN '2018-04-01' AND '2019-04-01'; -- YYYY-MM-DD
...
+---------------+-------------------+ */
-- Type your code below:
-- select top 3 * from Sales order by Price asc
...
top_3_dir AS (
SELECT 
    name, COUNT(g.movie_id) AS movie_count
FROM
    names n
        INNER JOIN
    ...
    genre g ON m.id = g.movie_id
        INNER JOIN
    ratings r ON r.movie_id = m.id
WHERE genre IN (select genre from top_3_gen)
    AND avg_rating > 8
GROUP BY name
...
select *,
	SUM(avg_duration) OVER d1 AS running_total_duration,
    AVG(avg_duration) OVER d2 AS moving_avg_duration
FROM genre_duration
WINDOW d1 AS (ORDER BY genre ROWS UNBOUNDED PRECEDING),
d2 AS (ORDER BY genre ROWS UNBOUNDED PRECEDING);
...
SELECT genre, 
		year, 
        title as movie_name, 
	    REPLACE(REPLACE(worlwide_gross_income,'$', ''), 'INR', '') as worlwide_gross_income, -- removing $ and INR sign 
...
SELECT name_id, 
    ...
    duration,
    date_published,
    LEAD(date_published,1) OVER(PARTITION BY name_id ORDER BY date_published) AS next_day
FROM NAMES n
...
ROUND(AVG(DATEDIFF(next_day, date_published))) AS total_day,
